...
USample3DAttributesComponent::USample3DAttributesComponent()
{
...
void USample3DAttributesComponent::BeginPlay()
{
	Super::BeginPlay();
}

void USample3DAttributesComponent::TickComponent(float DeltaTime, ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
	Super::TickComponent(DeltaTime, TickType, ThisTickFunction);
}

#if WITH_EDITOR
void USample3DAttributesComponent::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
	Super::PostEditChangeProperty(PropertyChangedEvent);

	if (PropertyChangedEvent.MemberProperty->GetFName() == GET_MEMBER_NAME_CHECKED(USample3DAttributesComponent, AttributeType))
	{
...
void USample3DAttributesComponent::Setup3DAttributes(UArcGIS3DObjectSceneLayer* Layer)
{
...
void USample3DAttributesComponent::Setup3DAttributesFloatAndIntegerType(UArcGIS3DObjectSceneLayer* Layer)
{
	...
	Layer->SetMaterialReference(
		LoadObject<UMaterial>(this, TEXT("Material'/ArcGISMapsSDK/Samples/3DAttributesSample/ConstructionYearRenderer.ConstructionYearRenderer'")));
}

// This function is an example of how to use attributes WITH the attribute processor
void USample3DAttributesComponent::Setup3DAttributesOtherType(UArcGIS3DObjectSceneLayer* Layer)
{
	// We want to set up an array with the attributes we want to forward to the material
	...
	// In general, you can use this function in other scripts to change the material thats used to render the buildings
	Layer->SetMaterialReference(
		LoadObject<UMaterial>(this, TEXT("Material'/ArcGISMapsSDK/Samples/3DAttributesSample/BuildingNameRenderer.BuildingNameRenderer'")));
}

// ForEachString takes care of converting the attribute buffer into a readable string value
void USample3DAttributesComponent::ForEachString(const Esri::GameEngine::Attributes::ArcGISAttribute& attribute,
...
int32 USample3DAttributesComponent::IsBuildingOfInterest(const FAnsiStringView& buildingName)
{
