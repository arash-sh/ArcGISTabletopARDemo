...
UArcGISViewStateLoggingComponent::UArcGISViewStateLoggingComponent() : Super()
{
	PrimaryComponentTick.bCanEverTick = false;
	bAutoActivate = true;
}

void UArcGISViewStateLoggingComponent::OnArcGISMapComponentChanged(UArcGISMapComponent* InMapComponent)
{
...
#if WITH_EDITOR
void UArcGISViewStateLoggingComponent::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
	Super::PostEditChangeProperty(PropertyChangedEvent);

	if (PropertyChangedEvent.MemberProperty->GetFName() == GET_MEMBER_NAME_CHECKED(UArcGISViewStateLoggingComponent, EnableLogging))
	{
		SetupViewStateEvents();
	}
}
...
void UArcGISViewStateLoggingComponent::SetupViewStateEvents()
{
	...
	auto arcGISViewChanged = [this](Esri::GameEngine::View::State::ArcGISViewState& state) {
	...
	auto arcGISLayerViewChanged = [this](Esri::GameEngine::Layers::Base::ArcGISLayer& layer,
	...
	auto arcGISElevationSourceViewChanged = [this](Esri::GameEngine::Elevation::Base::ArcGISElevationSource& elevation,
	...
	// Set event listeners
	MapComponent->GetView()->APIObject->SetViewStateChanged(std::move(arcGISViewChanged));
	MapComponent->GetView()->APIObject->SetLayerViewStateChanged(std::move(arcGISLayerViewChanged));
	MapComponent->GetView()->APIObject->SetElevationSourceViewStateChanged(std::move(arcGISElevationSourceViewChanged));

	return;
}

void UArcGISViewStateLoggingComponent::LogElevationStatus(Esri::GameEngine::View::State::ArcGISElevationSourceViewStatus status,
...
void UArcGISViewStateLoggingComponent::LogLayerStatus(Esri::GameEngine::View::State::ArcGISLayerViewStatus status,
													  FString layerName,
													  FString message,
													  FString additionalMessage)
{
...
void UArcGISViewStateLoggingComponent::LogViewStatus(Esri::GameEngine::View::State::ArcGISViewStatus status,
													 FString message,
													 FString additionalMessage)
{
	if (!EnableLogging)
...
//Error = 16,
//Warning = 32
FString UArcGISViewStateLoggingComponent::ElevationStatusToString(Esri::GameEngine::View::State::ArcGISElevationSourceViewStatus status,
																  bool* warning,
																  bool* error)
{
	*warning = false;
...
//Error = 16,
//Warning = 32
FString UArcGISViewStateLoggingComponent::LayerStatusToString(Esri::GameEngine::View::State::ArcGISLayerViewStatus status, bool* warning, bool* error)
{
	*warning = false;
	*error = false;
	auto bits = static_cast<int>(status);
...
//Error = 16,
//Warning = 32
FString UArcGISViewStateLoggingComponent::ViewStatusToString(Esri::GameEngine::View::State::ArcGISViewStatus status, bool* warning, bool* error)
{
	*warning = false;
	*error = false;
	auto bits = static_cast<int>(status);
